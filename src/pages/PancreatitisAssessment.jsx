    ...
    return (
        <div className={`print-section border-2 ${border} rounded-[10px] overflow-hidden bg-white mb-4 break-inside-avoid print:mb-3`}>
            <div className={`${color} text-white px-4 py-2 flex justify-between items-center print:text-black print:border-b-2 print:border-black print:py-1`} style={{WebkitPrintColorAdjust: 'exact'}}>
                <div>
                    <h3 className="text-base font-bold print:text-sm">{group.name}</h3>
                ...
                <div className="text-right">
                    <div className="text-2xl font-bold print:text-xl">{total}</div>
...
const PancreatitisAssessment = (props) => {
    ...
    return (
        <div className="space-y-8 animate-fade-in pb-20 font-[Inter,system-ui,sans-serif]">
            {/* INPUTS */}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex flex-col gap-2 mb-4 bg-rose-50/50 p-4 rounded-[8px] border border-rose-100 print:border-black print:bg-transparent">
                            ...
                            <label className="flex items-center gap-2 text-sm text-[#2d2926] cursor-pointer">
                                <input type="checkbox" checked={apCriteria.pain} onChange={(e) => { const n = {...apCriteria, pain: e.target.checked}; setApCriteria(n); syncToParent(undefined,undefined,undefined,undefined,n); }} className="w-4 h-4 text-rose-600 border-[#d6d0c8] rounded focus:ring-rose-500"/>
                                Abdominal pain consistent with AP
                            </label>
                            <label className="flex items-center gap-2 text-sm text-[#2d2926] cursor-pointer">
                                <input type="checkbox" checked={apCriteria.lab} onChange={(e) => { const n = {...apCriteria, lab: e.target.checked}; setApCriteria(n); syncToParent(undefined,undefined,undefined,undefined,n); }} className="w-4 h-4 text-rose-600 border-[#d6d0c8] rounded focus:ring-rose-500"/>
                                Serum lipase or amylase &gt; 3 times UNL
                            </label>
                            <label className="flex items-center gap-2 text-sm text-[#2d2926] cursor-pointer">
                                <input type="checkbox" checked={apCriteria.imaging} onChange={(e) => { const n = {...apCriteria, imaging: e.target.checked}; setApCriteria(n); syncToParent(undefined,undefined,undefined,undefined,n); }} className="w-4 h-4 text-rose-600 border-[#d6d0c8] rounded focus:ring-rose-500"/>
                                Characteristic findings from imaging (CECT or MRI)
                            </label>
                            
                            <div className={`mt-3 p-2 rounded text-xs font-bold text-center ${isApConfirmed ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-[#f4f2ee] text-[#a8a099] border border-[#ebe8e2]'}`}>
                    ...
                    <div className="bg-rose-50/30 p-4 rounded border border-rose-100 print:bg-transparent print:border-black">
                        ...
                        <div className="print:hidden mb-4 border-b border-rose-200 pb-4">
                            ...
                            <button onClick={addBatchLabs} className="w-full bg-rose-600 text-white px-4 py-2 rounded text-sm font-bold hover:bg-rose-700 shadow-[0_1px_4px_rgba(0,0,0,0.05)]">
            ...
            {isAnalyzed && timelineData && (
                <div className="animate-fade-in space-y-8">
                    <div className="bg-white rounded-[10px] border border-[#ebe8e2] shadow-[0_1px_4px_rgba(0,0,0,0.05)] p-6 overflow-hidden break-inside-avoid page-break-inside-avoid print:border-black print:shadow-none relative flex flex-col">
                        <div className="text-xs font-bold text-[#a8a099] uppercase mb-4 tracking-wider flex items-center gap-2 print:text-black"><span className="w-2 h-4 bg-rose-500 rounded-sm print:bg-black"></span> Timeline Visualization</div>
                                ...
                                {onset && (
                                    <div className="absolute top-0 bottom-0 left-[150px] right-0 pointer-events-none z-0">
                                        <div className="absolute top-[-30px] bottom-0 w-0.5 bg-rose-500 border-l border-dashed border-rose-500 print:border-black -translate-x-1/2" style={{left:`${timelineData.getPos(onset)}%`}}>
                                            <div className="absolute -top-6 -left-0 -translate-x-1/2 bg-rose-600 text-white text-[9px] px-2 py-0.5 rounded shadow-[0_1px_4px_rgba(0,0,0,0.05)] font-bold whitespace-nowrap print:bg-transparent print:text-black print:border print:border-black" style={{WebkitPrintColorAdjust:'exact'}}>Onset {formatDate(onset)}</div>
                                        </div>
                                    </div>
                                )}
                    ...
                    <div className="print:break-before-auto">
                        <div className="flex flex-wrap gap-2 mb-4 border-b pb-1 print:hidden">{uniqueDrugs.map(d=>{
                            ...
                            return <button key={d.id} onClick={()=>setActiveTab(k)} className={`px-4 py-2 rounded-t-lg border-t border-x border-b-0 text-sm font-bold ${activeTab===k?'bg-white text-rose-600 -mb-px z-10':'bg-[#faf9f7] text-[#a8a099]'}`}>{d.name} ({total})</button>
                        })}</div>
